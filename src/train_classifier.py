...
def evaluate(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for images, labels in loader:
            ...
            # Combo accuracy: hele kombinasjonen må være riktig
            correct_combo += (preds == labels).all(dim=1).sum().item()
    ...
    combo_acc = correct_combo / max(1, total)

    return avg_loss, helmet_acc, vest_acc, glasses_acc, combo_acc
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        val_loss, helmet_acc, vest_acc, glasses_acc, combo_acc = evaluate(
